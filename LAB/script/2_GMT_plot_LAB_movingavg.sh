...
#Inputs=Lat_lon_LABDEPTH_moving_average_window_*.txt
...
input=Lat_lon_LABDEPTH_moving_average_window_7.txt
...
out="SE_Asia_LAB_distribution_${pip}"
...
gmt makecpt -Cjet -I -Z -T$dp_min/$dp_max/$incre > $out.cpt
...
#Plotting
echo "plotting..."
gmt psbasemap $proj $bounds -Bxa -Bya -BWSen+t"LAB($input)" -K > $out.ps
...
gmt psscale $bounds $proj -DjCT+w6i+o10.5/0.1c+v+e -C$out.cpt -Bxa -By+l"LAB depth (km)" -O -K >> $out.ps
...
gmt psxy ${dig}/tmp_Sag.txt -JM -R -W0.8p,blue -K -O >> $out.ps
