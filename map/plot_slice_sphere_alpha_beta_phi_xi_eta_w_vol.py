...
for irow in range(nrow):
  ...
  m.drawparallels(map_parallels, linewidth=0.1, labels=[1,0,0,0], fontsize=6)
  m.drawmeridians(map_meridians, linewidth=0.1, labels=[0,0,0,1], fontsize=6)
  
  # contourf model
  xx, yy = m(lons2, lats2)

  if model_tag in ['alpha', 'alpha_smooth']:
    ...
    levels = np.concatenate((np.arange(-5,0,0.5), np.arange(0.5,5.1,0.5)))
    ...
    cb = m.colorbar(cs,location='right',ticks=np.arange(-5,5.1,1), pad="5%")
    cb.ax.set_title('(%)', fontsize=10)
    #cb.set_label('%')
    ax.set_title("{:s}".format(model_tag))
  
  elif model_tag in ['beta', 'beta_smooth']:
    zz = np.transpose(model[model_tag])
    zz = zz * 100 # use percentage
  ...
  elif model_tag in ['eta', 'eta_smooth']:
    zz = np.transpose(model[model_tag])
  ...
  elif model_tag in ['phi', 'xi', 'phi_smooth', 'xi_smooth']:
    zz = np.transpose(model[model_tag])
    zz = zz*100.0 
    ...
    # colorbar for contourfill
    levels = np.arange(0.95, 1.05, 2)
    cb = m.colorbar(cs,location='right',pad="5%", format="%.2f", ticks=levels)
    cb.ax.set_title('SH/SV', fontsize=10)
    ax.set_title("{:s}".format(model_tag))
  ...
  elif model_tag in ['alpha_kernel_smooth', 'beta_kernel_smooth', 'eta_kernel_smooth', 'phi_kernel_smooth', 'xi_kernel_smooth', 'alpha_dmodel', 'beta_dmodel', 'eta_dmodel', 'phi_dmodel', 'xi_dmodel', 'hess_kernel_sq_smooth_sqrt', 'alpha_kernel_precond_smooth', 'beta_kernel_precond_smooth', 'alpha_kernel', 'beta_kernel']:
    zz = np.transpose(model[model_tag])
    #scale = np.mean(model[model_tag])
    #max_n = np.max(model[model_tag])
    #print(max_n)
    #zz = zz/scale/10.0
    zz = zz*5000
    levels = np.arange(-5, 5.01, 1)
    ...
    levels = np.arange(-5, 5.01, 1)
    cb = m.colorbar(cs,location='right',pad="5%", format="%.2f", ticks=levels)
    cb.ax.set_title('10^4', fontsize=10)
    ax.set_title("{:s}".format(model_tag))

  elif model_tag in ['alpha_inv_hess_diag', 'beta_inv_hess_diag', 'alpha_Hdm_sq_smooth_sqrt', 'beta_Hdm_sq_smooth_sqrt']:
    zz = np.transpose(model[model_tag])
    #max_n = np.max(model[model_tag])
    #print(max_n)
    zz = zz*300000
  ...
  elif model_tag in ['alpha_dmodel_scale', 'beta_dmodel_scale', 'eta_dmodel_scale']:
    zz = np.transpose(model[model_tag])
    zz = zz*1000.0*0.7
    levels = np.arange(-5, 5.01, 1)
    cmap = plt.cm.get_cmap("jet_r")
    ...
    cb = m.colorbar(cs,location='right',pad="5%", format="%.1f", ticks=levels)
    cb.ax.set_title('10^4', fontsize=10)
    ax.set_title("{:s}".format(model_tag))
  elif model_tag in ['vsv', 'vsh', 'vpv', 'vph', 'vp0', 'vs0']:
    zz = np.transpose(model[model_tag])
    vmean = np.mean(model[model_tag])
    #levels = np.linspace(0.95*vmean, 1.05*vmean, 11)  ##CL, but not appropriate for the upper 50 km;v1
    #levels = np.linspace(0.9*vmean, 1.1*vmean, 11)  ##RB;v2
    levels = np.linspace(0.75*vmean, 1.25*vmean, 11)
    cmap = plt.cm.get_cmap("jet_r")
    cs = m.contourf(xx, yy, zz, cmap=cmap, levels=levels, extend="both")
    cs.cmap.set_over('black')
    cs.cmap.set_under('purple')
  ...
  for l in block_lines:
    x, y = m(l[0], l[1])
    ax.plot(x, y, lw=0.2, color='white')

  #-- plot plate_boundary
  #pb_line_file = 'trench.txt'  ##trench.txt gives me an error
  pb_line_file = 'zhangpz_pb.txt'  ##RB added
  pb_lines = []
  ...
  for l in pb_lines:
    x, y = m(l[0], l[1])
    ax.plot(x, y, lw=0.6, color='red')

  #--- plot seismicity
 # catalog_file = 'isc.txt'
 ...
 # ax.plot(x, y, 'w.', markersize=markersize)
  ...
  #ax.plot(x, y, '^', markeredgecolor='red', markerfacecolor='none', markersize=2, markeredgewidth=0.5)


#RB plotting SE volcanoes

filename = 'volcanoes_RB.txt'
with open(filename,'r') as file:

	for line in file:
		row=line.split()  #reads each line from the txt file
		lon=float(row[1])
		lat=float(row[0])

		x,y = m(lon,lat)
#               print(x,y)
		ax.plot(x,y, '^', color='red', markersize='4', markeredgecolor='white')
