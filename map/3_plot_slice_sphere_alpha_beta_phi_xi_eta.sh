...
#sem_utils=/scratch/04591/geostar/Ani/sem_utils
...
do
  ...
  python3 ./plot_slice_sphere_alpha_beta_phi_xi_eta_w_vol.py \
    $nc_dir/${nc_tag}.nc "$title ($depth km)" $model_names $out_fig
done

cd $out_dir
gs -dNOPAUSE -sDEVICE=pdfwrite -sOUTPUTFILE=model.pdf -dBATCH *.pdf
rm depth_*.pdf
